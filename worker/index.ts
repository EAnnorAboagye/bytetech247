...
// `cf-cache-status: HIT`). Deriving the nonce from a secret + a time
// bucket instead means every request within the same window — cached or
// freshly computed — agrees on the same value, so caching keeps working
// unmodified while the nonce still rotates periodically rather than
// staying fixed forever the way the old hash-based CSP did.
//
// Was 5 minutes; widened to 60 after a live-confirmed gap (2026-08-15):
// the CSP header is only ever re-read by the browser on a real top-level
...
// the browser is still actually enforcing — confirmed live by loading
// the homepage, waiting past a 5-minute boundary, then clicking an
// in-page link and diffing the enforced vs. newly-fetched nonce values.
// A 5-minute window made that mismatch common on any real browsing
// session; 60 minutes doesn't eliminate the structural gap (only a
// server-driven hard-reload-on-mismatch or abandoning nonce rotation
// entirely would), but makes crossing it during a normal visit rare
// while still rotating far more often than the old permanent hash-based
// policy did.
...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    if (
      ...
      // `pagefind --site dist` postbuild step — see CommandPalette.astro)
      // uses idiosyncratic extensions of its own (.pagefind, .pf_meta,
      // .pf_fragment, ...) that STATIC_ASSET_EXTENSION never tried to
      // enumerate. Confirmed live (2026-08-15): without this exclusion,
      // a request for /pagefind/wasm.en.pagefind got 301'd to
      // .../wasm.en.pagefind/ — a URL with no matching asset — which is
      // what broke search sitewide the moment this redirect shipped
      // (pagefind.js's own fetch calls don't expect a redirect there, and
      // whatever it received instead failed as "invalid gzip data").
      // Excluded wholesale, the same way /api/ already is, rather than
      // trying to keep pace with every extension Pagefind might ever use.
