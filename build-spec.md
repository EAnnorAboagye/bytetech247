...
- **Primary nav (≥72rem/1152px viewports):** Home, then one real `<a href>` per locked category from §9 (`Dev Tools`, `Data & Automation`, `AI Productivity`, `Guides & Fixes`) — this list only, matching what this section always specified. Each category is paired with a `<details>/<summary>` disclosure revealing its most-used tags (computed at build time from that category's posts, e.g. Dev Tools surfaces Docker, Git, Cloud Hosting, AWS — see Phase 2) plus a "View all in [Category]" link to the archive. Native HTML disclosure, not a JS dropdown — every link is a real `<a href>` in the DOM regardless of open/closed state, so it's keyboard-operable and fully crawlable with zero JS (Phase 7 baseline). About/Contact/every legal page are deliberately *not* in this nav — they're already in the footer and in the command palette's static-page list (Phase 5/8), and adding them here (a later, undocumented drift from this original spec) is what overflowed the nav at ordinary desktop widths; removed 2026-08-04 to restore what this section always said.
- **Right:** search trigger (opens the ⌘K command palette, Phase 5/8), dark-mode toggle (Phase 5).
- **Below 72rem/1152px (phones, tablets, and narrower laptops):** menu icon positioned left, before the logo (editorial convention). Tapping it opens the collapsed nav — Home, categories with the same tag disclosures as desktop, *plus* About, Contact, and the Privacy/legal-pages disclosure, since there's no width constraint forcing the same trim on this size class. Search and dark-mode toggle stay visible on the right, outside the collapse. The 1152px threshold (not a smaller "tablet" breakpoint) is deliberate: with the full 8-item nav it used to carry, the horizontal layout didn't actually have room until viewports at least that wide — confirmed by measuring the real rendered width (logo + nav + actions ≈ 1152px at minimum) rather than picking a breakpoint by feel.
...
- **Logo/favicon assets** — designed and committed at repo root under `brand-assets/`: `favicon.svg` (32×32, simplified mark, no status dot — kept ultra-legible at 16–32px), `logo-mark.svg` (128×128, full mark with amber status dot, for app icons/social avatars/`Organization.logo`). Brand teal `#0D9488`, accent amber `#F59E0B`. Google accepts SVG directly for `Organization.logo` (min. 112×112 — `logo-mark.svg` at 128×128 clears this), but also export a PNG fallback (e.g. 512×512) during Phase 0/10 asset setup for universal compatibility with tools that don't parse SVG.
  - A horizontal lockup file (`logo-full.svg`, mark + "ByteTech247" wordmark) was originally designed for the site header but never actually wired in — `Header.astro` renders `logo-mark.svg` as a small icon next to a live `<span>{siteConfig.name}</span>` instead, which gets the same visual result with real crawlable text rather than text baked into an SVG. The unused file was removed 2026-08-14 as a dead-asset cleanup; if a static wordmark lockup image is ever wanted instead of the live-text approach, redesign and recommit it deliberately rather than assuming this old file is still current.
