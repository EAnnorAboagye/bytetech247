...
All 10 clusters approved by user — drafting in progress. Writing order: 11-20 in sequence. Flip each entry's Status to `written` and fill in its Slug as its MDX file is created, per the pillar-cluster/guides-fixes-article workflow — this is what lets the batch resume correctly if the session ends mid-batch.
