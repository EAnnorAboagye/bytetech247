...
4. **Body sections (`##`/`###`)** — one real sub-question per heading, in the order a reader would actually ask them. Each section makes exactly one point and backs it with a command, a number, or a concrete example — never "many developers find that...". Use the site's actual MDX components where they earn their place, not by default:
   - `<Callout type="info|tip|warning|danger">` for a genuinely separable aside (a gotcha, a caveat) — not for restating the paragraph above it. `type="info"` renders labeled "Note"; there is no separate `"note"` type — `src/components/Callout.astro`'s `Props.type` union is exactly `"info" | "tip" | "warning" | "danger"`, and passing anything else throws `Cannot read properties of undefined` at render time (confirmed live 2026-08-19 across 14 posts that had used the since-corrected `type="note"`, all fixed to `type="info"`).
...
## 3. Cover image sourcing (Pexels, or generated abstract art)
...
**When Pexels doesn't have a genuine fit** for an otherwise-conceptual topic (searched, nothing on-topic came back), generate an abstract illustration instead of settling for a generic/off-topic stock photo:

1. Run `node scripts/generate-conceptual-cover.mjs <post-folder>/cover.jpg "<subject prompt>"` (uses Cloudflare Workers AI's `flux-1-schnell`, credentials from `.dev.vars` — see that script's own comment). The subject prompt describes the post's concept only (e.g. "a glowing neural network of interconnected nodes, dark background, teal accent lighting") — never mention text, UI, a screenshot, code, or data, since the script already forces a fixed negative-constraint suffix onto every prompt that rules those out structurally, not just by asking nicely.
2. **This is strictly for abstract/illustrative imagery.** Never use it for anything that could be mistaken for a real captured artifact — no fake terminal output, no fake API response, no fake screenshot, no fake chart. Every category skill's own cover-image section (§10/§11 there) still requires the terminal-capture script or the hand-built data-driven SVG for those cases; this AI path never substitutes for those, only for the Pexels-conceptual-photo case.
3. Open the generated image and check it by eye before committing: confirm there's no garbled pseudo-text or UI-like artifact in it (a known diffusion-model failure mode) — regenerate with a adjusted prompt if there is, don't ship a cover with illegible text-like scribbles baked into it.
4. Write `coverImageAlt` describing the actual abstract image, same as any other cover. No `coverImageCredit` — that field is for crediting a real photographer, and there isn't one here.
...
- [ ] **If this post is one of an approved batch from `pillar-cluster`** (check `.claude/content-plans/<category>.md`): `series` (the pillar's title, or the shared value that plan recorded) and `seriesOrder` (this cluster's position) are set in frontmatter — not optional, not "if it seems relevant." This is what makes `src/pages/[category]/[slug].astro` render the "Series progress (N of M)" navigation linking every cluster in the batch together; skipping it leaves the post structurally orphaned from its own cluster, confirmed live as a real gap across 30 posts published before this checklist item existed. Also set `relatedSlugs` to point at the pillar's own article once it's been written, so the interlink runs both directions.
