...
faq:
  - question: "Why doesn't a native <details> dropdown close when you click outside it?"
    answer: "The HTML spec only wires up one interaction for <details>: toggling via its own <summary>. For a plain collapsible section (an FAQ answer, a spoiler), staying open no matter where else a reader clicks is the correct behavior, and the browser has no way to know a page is using it as a dropdown menu instead."
  - question: "How do you make a <details> dropdown close on outside click and Escape?"
    answer: "Add a document-wide click listener that closes any open <details> unless the click happened inside that same element, and a separate keydown listener that closes every open <details> when Escape is pressed, regardless of where focus currently sits."
---
...
`<details>` only wires up one interaction: toggling via its own `<summary>`. Fix outside-click-to-close by adding a document-wide `click` listener that closes any open `.site-header details[open]` unless the click happened inside that element, plus a `keydown` listener that closes every open one on Escape. No dependency, about ten lines total.
