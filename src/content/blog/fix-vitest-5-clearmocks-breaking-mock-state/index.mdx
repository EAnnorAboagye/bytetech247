...
<Callout type="warning" title="This changes what 'passing' means for the same test">
  The guide's own before/after example makes the shift concrete: a second
  test that used to see `toHaveBeenCalledTimes(2)` (because the first test's
  call was still counted) now sees `toHaveBeenCalledTimes(1)`. Nothing about
  the mock or the test code changed. Only the default that decides whether
  history survives between tests changed.
</Callout>
