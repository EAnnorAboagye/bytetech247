...
faq:
  - question: "Why does Astro merge two expressions like {count} and {label} into '5posts' with no space?"
    answer: "Astro's JSX-derived template syntax trims whitespace-only text — just spaces, tabs, or newlines — between two expressions instead of preserving it as a space. When nothing but a line break separates {count} and {label}, that line break is treated as pure indentation and discarded, so the rendered output has no space at all."
  - question: "How do you fix a missing space between two Astro template expressions?"
    answer: 'Add an explicit {" "} expression where the space should be. A string literal containing one space is a real value, not whitespace-only text, so Astro renders it exactly as written regardless of how the surrounding lines wrap.'
---
