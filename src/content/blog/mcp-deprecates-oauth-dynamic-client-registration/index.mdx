...
<Callout type="warning" title="DCR still works, this isn't a hard break yet">
  ...
  per the changelog, so an existing DCR-based integration isn't broken today.
  Treat this as a signal to plan the CIMD migration, not an emergency fix,
  unless you're building new client integrations, where CIMD should be the
