...
<Callout type="tip" title="Read this as an opportunity, not just a fix">
  For a project that always meant to share the root's Vite plugins and
  aliases, the new default removes config that was arguably always
  boilerplate. Only add `extends: false` to the projects where isolation was
  the actual intent, not to every project reflexively.
</Callout>
